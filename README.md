...
- **Автоматический fallback**: если FCM не работает, используются локальные уведомления
- **Надежная доставка**: уведомления приходят всегда при новых сообщениях

## Диагностика проблем

Если уведомления не приходят, запустите в консоли браузера:

```javascript
diagnosePushNotifications()
```

Эта функция проверит все компоненты системы и покажет, где проблема.
